    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-['Plus_Jakarta_Sans']">
      ...
      {(isQuickAddOpen || editingTransaction) && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
          ...
          <div className="bg-white w-full max-w-2xl rounded-t-[2rem] md:rounded-[2.5rem] shadow-2xl relative overflow-hidden animate-fadeIn h-[90vh] md:h-auto overflow-y-auto">
            ...
            <form onSubmit={handleAdminLogin} className="p-8 space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="••••••••"
                  className={`w-full px-5 py-4 bg-slate-50 border-2 rounded-2xl outline-none font-bold transition-all ${loginError ? 'border-rose-500 bg-rose-50' : 'border-slate-100 focus:border-indigo-500'}`}
                  autoFocus
                />
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 active:scale-95">Masuk Sekarang</button>
            </form>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 md:ml-72 flex flex-col min-h-screen">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 md:px-12 py-4 sticky top-0 z-40 flex justify-between items-center h-16 md:h-20">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <h2 className="text-xs font-black text-slate-800 uppercase tracking-tighter md:text-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex bg-slate-50 border border-slate-100 px-4 py-2 rounded-xl items-center gap-2">
               <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
               <span className="text-[11px] font-black text-slate-600">Rp {summary.balance.toLocaleString('id-ID')}</span>
            </div>
            
            {/* Quick Admin Access Toggle for Mobile */}
            {!isAdmin ? (
              <button onClick={() => setIsLoginModalOpen(true)} className="md:hidden p-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-indigo-100 hover:text-indigo-600 transition-all">
                <Lock className="w-5 h-5" />
              </button>
            ) : (
              <button onClick={() => setActiveTab('admin')} className={`md:hidden p-2.5 rounded-xl transition-all ${activeTab === 'admin' ? 'bg-purple-600 text-white shadow-md' : 'bg-purple-100 text-purple-600'}`}>
                <ShieldCheck className="w-5 h-5" />
              </button>
            )}
            
            <button onClick={() => setActiveTab('settings')} className="w-10 h-10 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 hover:border-indigo-500 transition-all">
        ...
        <main className="p-6 md:p-12 animate-fadeIn flex-1 pb-32 md:pb-12">
            ...
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-8">
                   <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100"><FinancialCharts transactions={transactions} /></div>
                   <div className="hidden md:block bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                     <h3 className="text-lg font-black mb-6 text-slate-800">Catat Baru</h3>
          ...
          {activeTab === 'transactions' && (
            <div className="space-y-8">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div>
                  <h2 className="text-3xl font-black text-slate-800 leading-tight">Arsip Keuangan</h2>
                  <p className="text-slate-400 font-medium">Lacak setiap rupiah yang Anda kelola.</p>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                   <button onClick={() => setIsQuickAddOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-4 rounded-2xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"><Plus className="w-5 h-5" /> Catat Kas</button>
                   <button onClick={() => { /* export logic */ }} className="p-4 bg-white text-indigo-600 rounded-2xl font-black border border-slate-100 hover:bg-slate-50 shadow-sm transition-all"><Download className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              <div className="flex flex-col lg:flex-row gap-4">
                <div className="flex-1 relative group">
                  ...
                  <input type="text" placeholder="Cari deskripsi atau kategori..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-14 pr-6 py-4 bg-white border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-500 transition-all font-bold shadow-sm" />
                </div>
                <div className="flex flex-col sm:flex-row gap-2">
                   <div className="relative flex-1">
                      <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                      <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full pl-11 pr-4 py-4 bg-white border-2 border-slate-100 rounded-2xl outline-none font-bold shadow-sm text-sm" />
                   </div>
                   <div className="flex items-center justify-center text-slate-300 hidden sm:flex"><ArrowRight className="w-4 h-4" /></div>
                   <div className="relative flex-1">
                      <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                      <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full pl-11 pr-4 py-4 bg-white border-2 border-slate-100 rounded-2xl outline-none font-bold shadow-sm text-sm" />
                   </div>
                   {hasActiveFilters && <button onClick={() => { setSearchTerm(''); setStartDate(''); setEndDate(''); }} className="px-5 py-4 bg-rose-50 text-rose-600 rounded-2xl font-black hover:bg-rose-100 transition-all active:scale-95"><FilterX className="w-5 h-5" /></button>}
                </div>
              </div>
          ...
          {activeTab === 'settings' && (
            <div className="max-w-xl mx-auto py-10 space-y-8">
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-indigo-50 to-transparent"></div>
                <div className="relative z-10">
                  <div className="w-24 h-24 bg-white text-indigo-600 rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-xl border-4 border-slate-50">
                    <User className="w-12 h-12" />
                  </div>
                  <h2 className="text-2xl font-black text-slate-800 mb-1">Profil Pengguna</h2>
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-8">Data Lokal Terenkripsi</p>
                  
                  <div className="space-y-4">
                    <button onClick={() => { /* export */ }} className="w-full p-5 bg-slate-50 rounded-[1.5rem] font-black text-slate-700 hover:bg-slate-100 transition-all flex justify-between items-center group">
                      <div className="flex items-center gap-4">
                        <div className="p-3 bg-white rounded-xl shadow-sm text-indigo-500 group-hover:scale-110 transition-transform"><Download className="w-5 h-5" /></div>
                    ...
                    {!isAdmin ? (
                      <button onClick={() => setIsLoginModalOpen(true)} className="w-full p-6 bg-slate-900 text-white rounded-[1.5rem] font-black hover:bg-slate-800 transition-all flex justify-between items-center shadow-xl shadow-slate-200 group active:scale-[0.98]">
                        <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-3">
                        <button onClick={() => setActiveTab('admin')} className="w-full p-6 bg-purple-600 text-white rounded-[1.5rem] font-black hover:bg-purple-700 transition-all flex justify-between items-center shadow-xl shadow-purple-100 group active:scale-[0.98]">
                          <div className="flex items-center gap-4">
                        ...
                        <button onClick={() => setIsAdmin(false)} className="w-full py-4 text-rose-500 font-black text-xs uppercase tracking-widest hover:bg-rose-50 rounded-xl transition-all">
              ...
              <p className="text-center text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em]">{appName} v2.0 • 2024</p>
            </div>
          )}
        </main>
      </div>
      ...
      <button 
        ...
        className="md:hidden fixed bottom-24 right-6 w-16 h-16 bg-indigo-600 text-white rounded-full shadow-2xl flex items-center justify-center z-50 hover:scale-110 active:scale-95 transition-all shadow-indigo-200"
      >
        <Plus className="w-8 h-8" />
      </button>

      {/* Mobile Bottom Navigation */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 flex justify-around p-4 z-[60] pb-8">
        {[
          ...
          <button 
            ...
            className={`p-3.5 rounded-2xl transition-all relative ${activeTab === item.id ? (isAdmin ? 'text-purple-600 bg-purple-50' : 'text-indigo-600 bg-indigo-50') : 'text-slate-400'}`}
          >
